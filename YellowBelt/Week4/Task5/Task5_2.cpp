...
pair<RandomIt, RandomIt> FindStartsWith(
    ...
    const string &prefix)
{
    const int pref_size = prefix.size();

    auto equal_range_pair = equal_range(range_begin, range_end, prefix,
                                        [&pref_size](const string &lhs, const string &rhs) {
                                            return (lhs.substr(0, pref_size) < rhs.substr(0, pref_size));
                                        });

    return equal_range_pair;
}
