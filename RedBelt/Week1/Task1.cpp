...
{
    ForwardIterator max_element_it = find_if(first, last, pred);
    if (max_element_it == last) // empty or flase sequence
        return last;

    auto max_element_value = *first;

    ForwardIterator find_it;    
    ForwardIterator current_first = first;

    while (true)
    {
        find_it = find_if(current_first, last, pred);
        if (find_it == last)
            break;
        if (*find_it > max_element_value)
        {
            max_element_it = find_it;
            max_element_value = *find_it;
        }
        current_first = next(find_it);
    }
