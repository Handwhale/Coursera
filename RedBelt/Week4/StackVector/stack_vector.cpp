...
void TestRangedBasedForLoop()
{
  StackVector<int, 5> v;

  for (int &item : v)
  {
    Assert(false, "Iteration through empty container");
  }

  for (size_t i = 1; i <= v.Capacity() - 2; ++i)
  {
    v.PushBack(i);
  }
  ...
  ASSERT_EQUAL(iteration_count, v.Size());
}
...
{
  {
    ...
    RUN_TEST(tr, TestRangedBasedForLoop);
  }
