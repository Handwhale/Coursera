...
// Не забудь указать флаг хотя бы -O1 иначе total dur = aprox 25 sec
...
public:
  ...
  {
    return _GetItem(airport).second;
  }

  // добавить данный элемент
  void Insert(TAirport airport)
  {
    _GetItem(airport).second++;
  }
  ...
  {
    return _airpors_holder;
  }

private:
  Items _airpors_holder;

  // Only for Get method
  const Item &_GetItem(const TAirport &airport) const
  {
    return _airpors_holder.at(static_cast<size_t>(airport));
  }

  Item &_GetItem(const TAirport &airport)
  {
    return _airpors_holder.at(static_cast<size_t>(airport));
  }
  ...
  {
    // Filling enum values
    auto en_size = static_cast<uint32_t>(TAirport::Last_);
    for (int i = 1; i < en_size; i++)
    {
      _airpors_holder[i].first = static_cast<TAirport>(i);
    }
  }
};
