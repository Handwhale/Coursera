...
stringstream GenerateText(size_t sentence_count)
{
  ...
  sentences.reserve(sentence_count);

  for (int sent_i = 0; sent_i < sentence_count; sent_i++)
  {
    bool first = true;
    string sent = "";
    for (int i = 0; i < rand_value(min_word, max_word); i++)
    {
      if (first)
        first = false;
      else
      ...
      sent += words[rand_value(0, words.size() - 1)];
    }
...
void TestSpeed()
{
  ...
  auto ss2 = GenerateText(50000);
  auto ss3 = GenerateText(50000);
  SearchServer srv(ss);

  vector<pair<istringstream, ostringstream>> str_data;
  ...
  {
    if (i == 20)
      srv.UpdateDocumentBase(ss2);

    if (i == 25)
      srv.UpdateDocumentBase(ss3);

    str_data.emplace_back(make_pair(istringstream("stupid and foolish I dropped the other where are you"),
                                    ostringstream()));
    srv.AddQueriesStream(str_data.back().first, str_data.back().second);
  }
}
